...
def getMin(data):
    return re.findall("\\d{1,2}(?=[-])",data)

def getMax(data):
    return re.findall("\\d{1,2}(?=\\s)",data)

def getKey(data):
    return re.findall("\\w(?=[:])",data)

def getPasswords(data):
    return re.findall("(?<=:\\s)\\w+", data)

def checkPasswordValidity(data):
    minimumList = getMin(data)
    maximumList = getMax(data)
    keyList = getKey(data)
    passwordList = getPasswords(data)
    result = 0

    for index in range(len(passwordList)):
        count = passwordList[index].count(keyList[index])
        if int(minimumList[index]) <= count <= int(maximumList[index]):
            result += 1

    return result


def checkPasswordValidity2(data):
    minimumList = getMin(data)
    maximumList = getMax(data)
    keyList = getKey(data)
    passwordList = getPasswords(data)
    ...
    for index in range(len(passwordList)):
        firstPosition = int(minimumList[index]) - 1
        secondPosition = int(maximumList[index]) - 1
        if passwordList[index][firstPosition] == keyList[index] and passwordList[index][secondPosition] != keyList[index]:
            result += 1
        elif passwordList[index][firstPosition] != keyList[index] and passwordList[index][secondPosition] == keyList[index]:
            result += 1

    return result

print(checkPasswordValidity2(DATA))
